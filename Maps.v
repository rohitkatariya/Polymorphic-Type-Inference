...
(* Definition get_all_ {A:Type} (my_map: string->A): list string := 
  match my_map with
  | t_empty _ => []
  | t_update m x v => x::(get_all_ m) 
  end .
  ["a" ;"b"]%string.

Compute get_all_ examplemap'. *)
...
Proof.
  ...
  unfold eqb_string.
  destruct (string_dec x x) as [H1|H2].
  - reflexivity.
...
Proof.
  ...
  - destruct(string_dec x2 x).
    -- rewrite <- e0 in e. exfalso. apply H. rewrite e. reflexivity.
    -- reflexivity.
  - destruct(string_dec x2 x).
    * reflexivity.
    * reflexivity.
...
Theorem update_same : forall (A : Type) (m : partial_map A) x v,
    ...
    (x |-> v ; m) = m.
Proof.
  intros A m x v H. unfold update. rewrite <- H. unfold t_update. unfold eqb_string. apply functional_extensionality. intros.
  destruct (string_dec x x0 ). rewrite e. reflexivity. reflexivity.
